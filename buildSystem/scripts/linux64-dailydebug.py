...
sys.path.append(SCRIPTDIR+'/../../buildScripts')
...
if len(checkedExamples)>0:
  ...
  extraBuildArgs.append('--forceBuild')

# uncomment this to run static code analyzis
# extraBuildArgs.append("--staticCodeAnalyzis")

# build and run all examples
ret=subprocess.call([SCRIPTDIR+"/../../buildScripts/build.py", "--buildSystemRun"]+extraBuildArgs+["--rotate", "20", "-j", "2", "--sourceDir", SRCDIR, "--prefix", SRCDIR+"/local",
  ...
  "--passToConfigure", "--with-lowram", "--enable-python", "--enable-debug", "--enable-shared",
  "--disable-static", "--with-qmake=qmake-qt5",
